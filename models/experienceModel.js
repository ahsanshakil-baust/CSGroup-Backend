...
module.exports = class ExperienceModel {
  ...
  static async deleteByPortfolioId(portfolio_id) {
    try {
      const snapshot = await db
        .collection(collectionName)
        .where("portfolio_id", "==", portfolio_id)
      ...
      if (snapshot.empty) {
        console.log(
          `No experience entries found for portfolio_id: ${portfolio_id}`
        );
        return;
      }
      ...
      await batch.commit();
      console.log(
        `All experience entries for portfolio_id ${portfolio_id} deleted successfully.`
      );
    } catch (error) {
      console.error(
        `Error deleting experience entries for portfolio_id ${portfolio_id}:`,
        error
      );
    }
  }
