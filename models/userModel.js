...
const range = "Sheet1!A:D"; // Assuming columns: ID, Name, Email, Password
...
module.exports = class User {
  ...
  save() {
    User.getAllUser((users) => {
      ...
      const updatedData = users.map((u) => [u.id, u.name, u.email, u.password]);
  ...
  static deleteUserById(id, callback) {
    User.getAllUser((users) => {
      ...
      const updatedData = updatedUsers.map((u) => [
        u.id,
        u.name,
        u.email,
        u.password,
      ...
      sheets.spreadsheets.values.update(
        ...
        (err, res) => {
          if (err)
            console.error("Error deleting user from Google Sheets:", err);
          else callback();
        }
      );
    });
